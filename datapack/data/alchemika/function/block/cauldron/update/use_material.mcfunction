...
$execute if entity @s[tag=alch.pure] unless score $cons.0 alch.dummy matches $(pure) run data modify entity @s item.components."minecraft:item_model" set value $(pure)
$tag @s add alch.$(tag)
