...
execute if predicate alchemika:random/one_half run particle minecraft:lava ~ ~0.2 ~ 0.2 0.2 0.2 0.5 10
...
$execute store result score $temp.existing1 alch.dummy run data get entity @s item.tag.alch_dat.cauldron[{id:$(out1_material_id)}].amount
$execute store result score $temp.existing2 alch.dummy run data get entity @s item.tag.alch_dat.cauldron[{id:$(out2_material_id)}].amount
...
$execute store result entity @s item.tag.alch_dat.cauldron[{id:$(in1_material_id)}].amount int 1 run scoreboard players get $temp.amount1 alch.dummy
$execute if score $temp.amount1 alch.dummy matches ..0 run data remove entity @s item.tag.alch_dat.cauldron[{id:$(in1_material_id)}]
$execute store result entity @s item.tag.alch_dat.cauldron[{id:$(in2_material_id)}].amount int 1 run scoreboard players get $temp.amount2 alch.dummy
$execute if score $temp.amount2 alch.dummy matches ..0 run data remove entity @s item.tag.alch_dat.cauldron[{id:$(in2_material_id)}]
$execute store result entity @s item.tag.alch_dat.cauldron[{id:$(in3_material_id)}].amount int 1 run scoreboard players get $temp.amount3 alch.dummy
$execute if score $temp.amount3 alch.dummy matches ..0 run data remove entity @s item.tag.alch_dat.cauldron[{id:$(in3_material_id)}]
