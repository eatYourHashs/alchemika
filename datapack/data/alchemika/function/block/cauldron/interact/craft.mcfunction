...
$execute store result score $temp.existing alch.dummy run data get entity @s item.components."minecraft:custom_data".alch_dat.cauldron[{id:$(material_id)}].amount
...
$execute if score $temp.existing alch.dummy matches 1.. store result entity @s item.components."minecraft:custom_data".alch_dat.cauldron[{id:$(material_id)}].amount int 1 run scoreboard players get $temp.amount alch.dummy
