...
$execute store result score $temp.amount alch.dummy run data get storage alchemika:registry recipes.scoop[{id:"$(id)"}].amount
$execute unless score $temp.amount alch.dummy matches 0 if score $temp.amount alch.dummy matches ..$(amount) run function alchemika:block/cauldron/interact/scoop/craft with storage alchemika:registry recipes.scoop[{id:"$(id)"}]
